...
var FloorplannerView = function(_floorplan, _viewmodel, _canvas) {

  var scope = this;
  var _floorplan = _floorplan;
  var _viewmodel = _viewmodel;
  var _canvas = _canvas;
  var canvasElement = document.getElementById(_canvas);
  var context = canvasElement.getContext('2d');

  // grid parameters
  var gridSpacing = ; // pixels
  var gridWidth = 1;
  ...
  //윈도우창 크기에 따른 요소 resize
  this.handleWindowResize = function() {
    var canvasSel = JQUERY("#"+_canvas);
  ...
  this.draw = function() {
    ...
    utils.forEach(_floorplan.getWalls(), drawWall);
    utils.forEach(_floorplan.getCorners(), drawCorner);
    if (_viewmodel.mode == _viewmodel.modes.DRAW) {
      drawTarget(_viewmodel.targetX, _viewmodel.targetY, _viewmodel.lastNode);
    }
    utils.forEach(_floorplan.getWalls(), drawWallLabels);
  }
  ...
  function drawWall(wall) {
    var hover = (wall === _viewmodel.activeWall);
    var color = wallColor;
    if (hover && _viewmodel.mode == _viewmodel.modes.DELETE) {
      color = deleteColor;
    } else if (hover) {
    ...
    drawLine(
      _viewmodel.convertX(wall.getStartX()),
      _viewmodel.convertY(wall.getStartY()),
      _viewmodel.convertX(wall.getEndX()),
      _viewmodel.convertY(wall.getEndY()),
  ...
  function drawEdgeLabel(edge) {
    ...
    context.strokeText(cmToFeet(length),
      _viewmodel.convertX(pos.x),
      _viewmodel.convertY(pos.y));
  ...
  function drawEdge(edge, hover) {
    ...
    drawPolygon(
      utils.map(corners, function(corner) {
        return _viewmodel.convertX(corner.x);
      }),
      utils.map(corners, function(corner) {
        return _viewmodel.convertY(corner.y);
      }),
      false,
      null,
      true,
  ...
  function drawCorner(corner) {
    var hover = (corner === _viewmodel.activeCorner);
    ...
    drawCircle(
      _viewmodel.convertX(corner.x),
      _viewmodel.convertY(corner.y),
  ...
  function drawTarget(x, y, lastNode) {
    drawCircle(
      _viewmodel.convertX(x),
      _viewmodel.convertY(y),
      cornerRadiusHover,
      cornerColorHover
    );
    if (_viewmodel.lastNode) {
      drawLine(
        _viewmodel.convertX(lastNode.x),
        _viewmodel.convertY(lastNode.y),
        _viewmodel.convertX(x),
        _viewmodel.convertY(y),
        wallWidthHover,
        wallColorHover
  ...
  function drawGrid() {
    ...
    var offsetX = calculateGridOffset(-_viewmodel.originX);
    var offsetY = calculateGridOffset(-_viewmodel.originY);
